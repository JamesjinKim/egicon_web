#!/usr/bin/env python3
"""
SDP810 센서 위치 찾기 스크립트
============================
모든 채널에서 SDP810 차압센서 검색 (0x25 주소)
"""
...
def scan_for_sdp810():
    """모든 채널에서 SDP810 센서 검색"""
    
    print("=" * 60)
    print("🔍 SDP810 센서 전체 검색")
    print("=" * 60)
    ...
    try:
                ...
                if channel_devices:
                    ...
                    for sdp_addr in sdp_addresses:
                        if sdp_addr in channel_devices:
                            print(f"   🎯 SDP810 후보 발견: 0x{sdp_addr:02X}")
                            
                            # SDP810 통신 테스트
                            success, pressure = test_sdp810_communication(bus, sdp_addr)
                            if success:
                                sensor_info = {
                                    'channel': channel,
                                    'address': sdp_addr,
                                ...
                                print(f"   ✅ SDP810 확인됨! 압력: {pressure:.2f} Pa")
                            else:
                                print(f"   ❌ SDP810 통신 실패")
                else:
                    print("   ❌ 장치 없음")
        ...
        if found_sensors:
            print(f"✅ 발견된 SDP810 센서: {len(found_sensors)}개")
            ...
            print("\n🔍 확인 사항:")
            print("   1. SDP810 센서가 실제로 연결되어 있는지")
            print("   2. 전원 공급이 정상인지 (3.3V 또는 5V)")
            print("   3. I2C 케이블 연결 상태")
            print("   4. 센서 주소 설정 (0x25 고정)")
...
def test_sdp810_communication(bus, address):
    """SDP810 센서 통신 테스트"""
    try:
        # 3바이트 읽기 시도
    ...
    try:
        ...
        for channel in range(8):
            try:
                ...
                # SDP810 통신 테스트
                success, pressure = test_sdp810_communication(bus, target_address)
                if success:
                    print(f"              🎯 SDP810 확인! 압력: {pressure:.2f} Pa")
...
if __name__ == "__main__":
    print("🚀 SDP810 센서 검색 시작")
    print(f"⏰ 시작 시간: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # 1. 전체 검색
    found_sensors = scan_for_sdp810()
    ...
    if found_sensors:
        print("✅ SDP810 센서 검색 성공!")
    else:
        print("❌ SDP810 센서를 찾지 못했습니다.")